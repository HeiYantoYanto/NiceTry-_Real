...
document.addEventListener('DOMContentLoaded', function() {
	...
	const quiz = [
		{ q: 'Bubble Sort has a worst-case time complexity of O(n²).', a: true },
		{ q: 'Quick Sort is always faster than Merge Sort.', a: false },
		{ q: 'Insertion Sort is efficient for large datasets.', a: false },
		{ q: 'Heap Sort is an in-place sorting algorithm.', a: true },
		{ q: 'Counting Sort is a comparison-based sorting algorithm.', a: false },
		{ q: 'Merge Sort is stable.', a: true },
		{ q: 'Selection Sort has a best-case time complexity of O(n).', a: false },
		{ q: 'Radix Sort processes digits from the most significant to the least significant.', a: false },
		{ q: 'Shell Sort is a variant of Insertion Sort.', a: true },
		{ q: ' Bucket Sort assumes the input is uniformly distributed.', a: true }
	];
	let current = 0;
	let correct = 0;
	let wrong = 0;
	...
	function nextQuestion() {
		...
		if (current < quiz.length) {
			...
			if (pass && typeof window.awardQuizPassXP === 'function') {
				window.awardQuizPassXP();
			}
