...
document.addEventListener('DOMContentLoaded', function() {
	...
	const quiz = [
		{ q: 'Linear Search can be used on both sorted and unsorted arrays.', a: true },
		{ q: 'Binary Search has a time complexity of O(n) in the worst case.', a: false },
		{ q: 'Interpolation Search is always faster than Binary Search.', a: false },
		{ q: 'Jump Search requires the array to be sorted.', a: true },
		{ q: 'Exponential Search is designed for unbounded or infinite arrays.', a: true },
		{ q: 'Ternary Search divides the array into two parts during each step.', a: false },
		{ q: 'Linear Search is more efficient than Binary Search for large sorted arrays.', a: false },
		{ q: 'Fibonacci Search uses Fibonacci numbers to determine the split points in the array.', a: true },
		{ q: 'Binary Search can be implemented both iteratively and recursively.', a: true },
		{ q: 'Interpolation Search requires comparisons to find the target element.', a: true }
	];
	let current = 0;
	let correct = 0;
	let wrong = 0;
